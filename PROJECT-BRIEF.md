# Project brief — "Four Nights in Nashville"
...
**Live Vercel target:** not yet deployed. See §9.
...
| Path | Bytes | Purpose |
| --- | --- | --- |
| `index.html` | ~81 KB | **The entire site.** Markup, CSS and JS in one file. No dependencies, no build step. |
| `vercel.json` | 487 | Static config: `cleanUrls`, `nosniff`, `Referrer-Policy`, no-cache on `index.html`. |
...
| `dist/artifact.html` | ~80 KB | Build output. Gitignored. |
...
look. The weekend is billed like a concert tour: *Four Nights in Nashville*,
...
| --- | --- | --- | --- |
| **E** (Elizabeth Mather) | Guest | Flies **MDW 8:00 AM Thu**, lands BNA **~9:20 AM** (~1h20, no time change). Home **6:40 PM Mon** from BNA, leave house ~4:15 PM. **Confirmed.** | — |
...
**Meal headcount is 7** (five guests plus Will and Hannah). Bruce does not go to
restaurants. This number appears in the Pelato booking, the Jack Brown's note,
and the lock list.

### Home base

**710 Buchanan St, Unit 5, Nashville, TN 37208** — Buchanan Arts District, just
north of Germantown. **Not in Germantown**, which an earlier draft wrongly
assumed; several walkability claims had to be corrected. Distances used:
...
| **Jack Brown's Beer & Burger Joint** | 1123 3rd Ave N, Germantown | **Walk-in only, no reservations.** Time not set |
| **Dinkville Under Broad** | Nashville Yards, beneath the Broad St railway overpass · (615) 910-3449 · 5 courts, **covered** · 9 holes mini golf · book via app/site/phone | **Venue chosen, court not booked** |
...
| **Printers Alley** | Between 3rd & 4th Ave N, off Church St, downtown · 5–10 min ride from Pelato | Saturday night plan |
...
**THU 3 SEP — Load-in**
- `~9:20 AM` E lands (8:00 AM MDW departure) — confirmed
...
- `12:00 – 1:30 PM` **Pickleball at Dinkville**, 90 minutes. Covered courts are
  why a noon start works in September
- `5:00 PM` **Pelato**, seven seats
- `7 PM onward` **Printers Alley.** Broadway as escalation, Germantown
  (Mother's Ruin, Germantown Café) as the bail-out
...
Eight buttons in `#whoGrid`, each `data-person="<id>"` with `aria-pressed`.
...
Not yet deployed — the environment this was built in had no Vercel credentials.
...
**Privacy note:** the page carries a home address and unit number alongside a
schedule showing when the house is empty (Sunday 7:30 AM, everyone leaves for
golf). That is fine for a link sent to family. Consider whether a public,
crawlable Vercel URL is the right exposure — options include a
`noindex` meta tag, Vercel password protection, or dropping the unit number.

---

## 10. Where to take it next

Ideas, roughly in value order. None are started.

1. **Shared live state.** The single biggest upgrade: let people RSVP to
   individual blocks, vote on the Jack Brown's time, or tick off the shopping
   list so everyone sees it. Currently all state is per-device `localStorage`.
   Needs a backend or a hosted form service.
2. **"Add to calendar."** Generate `.ics` for each block, or one file for the
   whole weekend. Purely client-side and genuinely useful.
